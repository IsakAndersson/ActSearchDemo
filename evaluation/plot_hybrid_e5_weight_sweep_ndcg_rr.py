...
DEFAULT_OUTPUT_DIR = EVALUATION_DIR / "plots" / "hybrid_e5_weight_sweep_ndcg_rr"
...
def _plot_metrics(results_df: pd.DataFrame, output_path: Path) -> None:
    ...
    for ax, (column_name, _, title, color) in zip(axes, METRIC_SPECS):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "information_need, then plot only nDCG and RR."
        )
    )
    parser.add_argument(
        "--qrels-path",
    ...
    csv_path = output_dir / "hybrid_e5_weight_sweep_ndcg_rr_metrics.csv"
    png_path = output_dir / "hybrid_e5_weight_sweep_ndcg_rr.png"
